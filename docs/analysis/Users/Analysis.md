...
- **GET /api/health**: Простой сервисный эндпоинт для проверки статуса системы (keep-alive). Используется для мониторинга внешними сервисами (например, UptimeRobot) и не требует JWT-токена.
